...
/**
 * This class implements all the methods of the Tokenizer interface, and defines
 * two new abstract methods, {@link #createBuffer} and {@link #fillBuffer} which
 * all concrete subclasses must implement. By default, instances of this class
 * can handle tokens of up to 16*1024 characters in length.
 * 
...
public abstract class AbstractTokenizer implements Tokenizer {
	...
	/**
	 * Create the {@link #text} buffer to use for parsing. This method may put
	 * text in the buffer, but it is not required to. In either case, it should
	 * set {@link #numChars} appropriately. This method will be called once,
	 * before tokenizing begins.
	 * 
	 * @param bufferSize
	 *            the minimum size of the created array, unless the subclass
	 *            knows in advance that the input text is smaller than this, in
	 ...
	 * method will be called when the tokenizer needs more characters to
	 ...
	 * tokenStart. It must preserve any characters after {@link #tokenStart} and
	 * before {@link #numChars}, however. After such a shift, it must adjust
	 * {@link #tokenStart}, {@link #tokenEnd} and {@link #p} appropriately.
	 * After the optional shift, the method should add as many new characters as
	 * possible to {@link #text} (and always at least 1) and adjust
	 * {@link #numChars} appropriately.
	 * 
	...
	public int next() throws IOException {
		...
		if ((skipSpaces || tokenizeSpaces) && Character.isWhitespace(c)) {
			...
			tokenEnd = p;
				...
				&& (wordRecognizer != null ? wordRecognizer.isWordStart(c)
			...
			do {
				...
				if (p >= numChars)
					...
					&& (wordRecognizer != null ? wordRecognizer.isWordPart(
							text[p], c) : Character
							.isJavaIdentifierPart(text[p])));
	...
	public int scan(char delimiter, boolean extendCurrentToken,
		...
		return scan(new char[] { delimiter }, false, extendCurrentToken,
				includeDelimiter, skipDelimiter);
	}
	...
	public int scan(String delimiter, boolean matchall,
		...
		return scan(delimiter.toCharArray(), matchall, extendCurrentToken,
				includeDelimiter, skipDelimiter);
	}

	protected int scan(char[] delimiter, boolean matchall,
			boolean extendCurrentToken, boolean includeDelimiter,
			boolean skipDelimiter) throws IOException {
